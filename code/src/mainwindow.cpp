...
MainWindow::MainWindow(QWidget *parent)
   ...
   */
  connect(ui->btnSort, SIGNAL(clicked()), this, SLOT(prepareSort()));
  connect(&sortWatcher, SIGNAL(finished()), this, SLOT(endSort()));
  connect(threadManager, SIGNAL(sig_incrementPercentComputed(double)), this,
...
/**
 * @brief MainWindow::prepareSort La fonction ci-dessous est exécutée à chaque
 * appui sur le bouton "Sort!"
 */
void MainWindow::prepareSort() {
  ...
  if (ui->inputSeqLen->text().toInt() <= 0)
    ...
    inputError = "Error: the sequence length must be less or equal than 15 "
                 "(15! = 1.3076744e+12 permutations).";
 ...
 */
#ifdef USING_QT5
  sortingAsync = QtConcurrent::run(threadManager, &ThreadManager::startSorting,
                                   seq, ui->inputThreads->text().toInt());
#elif USING_QT6
  sortingAsync = QtConcurrent::run(&ThreadManager::startSorting, threadManager,
                                   seq, ui->inputThreads->text().toInt());
#endif
  /*
...
/**
 * @brief MainWindow::incrementProgressBar La fonction ci-dessous est exécutée à
 * chaque réception d'un signal incrementPercentComputed en provenance de
 * threadManager
...
/**
 * @brief Cette fonction génère aléatoirement une séquence de nombres de la
 * taille requise
 * @param seqLen la taille de la séquence à trier
 * @param seed utilisé pour seeder la génération aléatoire de la séquence à
 * trier
 * @return séquence à trier
 */
std::vector<int> MainWindow::generateSequence(unsigned int seqLen,
                                              unsigned int seed) {
...
/**
 * @brief MainWindow::endSort La fonction ci-dessous est exécutée à la réception
 * d'un signal finished en provenance du sortWatcher, indiquant que la fonction
...
void MainWindow::endSort() {
  ...
  if (result.length() > 0) {
    msgBox.setText("Sequence is sorted!\nSequence: " + result +
                   "\nElapsed Time: " + QString::number(chronometer.elapsed()) +
                   " ms");
  } else {
